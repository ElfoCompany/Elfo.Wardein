...
{
     public class IISPoolWatcher : WardeinWatcher<IISPoolWatcherConfig>
     {
        protected IISPoolWatcher(IISPoolWatcherConfig config, string name, string group = null) : base(name, config, group)
        { }

        public static IISPoolWatcher Create(IISPoolWatcherConfig config, string group = null)
        {
            ...
            foreach (var service in Config.Services)
            {
                using (var persistenceService = ServicesContainer.PersistenceService(Const.DB_PATH))
                {
                    ...
                    IAmServiceManager GetServiceManager()
                    {
                        ...
                        try
                        {
                            svc = ServicesContainer.ServiceManager(service.ServiceName, ServiceManagerType.IISPool);
                        }
